...
- **Despliegue en Vercel retomado**: se creó `api/index.py` (wrapper ASGI que recorta el prefijo `/api`), `vercel.json` (build del frontend + rewrites) y `.python-version` (fija Python 3.11), y se corrigió `requirements.txt` (estaba en **UTF-16**, bug #11, un bug que habría roto el build de Vercel — ahora en UTF-8 plano). Verificado localmente: el wrapper responde correctamente contra la Neon real (`/api/health`, `/api/auth/login`) y `npm run build` del frontend genera `frontend/dist` sin errores. Decisiones tomadas con el usuario: `DEMO_MODE=true` en producción, misma BD de Neon de desarrollo, `JWT_SECRET` nuevo generado para producción (no versionado, ver sección 14).
- **Primer intento real de deploy en Vercel falló** (usuario lo ejecutó desde el dashboard, 2026-09-24): `Error: Function Runtimes must have a valid version`. Causa y fix: bug #12 (sección 12) — se quitó el campo `runtime` inválido de `vercel.json`, reemplazado por `.python-version`. **Falta reintentar el deploy** con el fix aplicado (requiere que el usuario haga commit/push y vuelva a desplegar desde el dashboard).
- **Siguiente tarea:** el usuario debe hacer commit + push de los cambios (incluido el fix del bug #12) y reintentar el deploy en Vercel (sección 14). Después: ejecutar la Fase 0 (pytest) cuando el usuario lo pida.
...
├── .python-version            # "3.11" — fija la versión de Python de la Vercel Function (ver bug #12 sección 12)
...
- La versión de Python de la function la fija `.python-version` (raíz, contenido `3.11`), **no** el campo `runtime` de `vercel.json` — ese campo espera un identificador de paquete con versión (ej. `now-php@1.0.0`), no un nombre de lenguaje suelto como `"python3.11"` (bug #12, sección 12).
